...
def mark_order_delivered(order_id, medio_pago, monto_pagado):
    ...
    nuevo_monto_pagado = monto_anterior_pagado + monto_pagado
    prod_paid = min(nuevo_monto_pagado, subtotal_products)
    rem = max(0.0, nuevo_monto_pagado - prod_paid)
    ...
    ws_pedidos.update_cell(row_idx, HEAD_PEDIDOS.index("Monto_pagado") + 1, nuevo_monto_pagado)
    ...
    append_row_ws(ws_flujo, [
        ...
        prod_paid - (subtotal_products - float(saldo_anterior)),
        domicilio_paid - (domicilio_monto - float(saldo_anterior)),
        saldo_total
    ])
