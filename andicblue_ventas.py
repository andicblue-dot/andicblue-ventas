...
try:
    ...
    # Si no, ReportLab usará una fuente por defecto que puede no tenerlos.
    try:
...
# --- AJUSTE: Mejora de rendimiento en escritura de Google Sheets ---
def safe_write_df_to_sheet(df: pd.DataFrame, sheet_title: str, headers: List[str]) -> bool:
    ...
    for attempt in range(5):
        try:
            ws.clear()
            # Usar update es MUCHO más rápido que un bucle de append_row
            ws.update(rows, 'A1')
...
# --- AJUSTE: Implementación de escritura atómica para prevenir corrupción de datos ---
...
# --- AJUSTE: Refactorización de edit_order para manejar el estado ---
def edit_order(order_id: int, new_items: Dict[str,int], new_domic_bool: bool=None, new_week: int=None, new_estado: str=None):
    ...
    if new_estado:
        df_ped.at[idx_h, "Estado"] = new_estado # <-- NUEVA LÓGICA
...
# ---------------------------
# NUEVO: MÓDULO DE FACTURACIÓN PDF
# ---------------------------
...
def get_next_invoice_number() -> int:
    ...
    try:
        ...
        return 0 # Devuelve 0 en caso de error grave
    finally:
        if lock_file.exists():
            lock_file.unlink()
...
def generate_invoice_pdf(order_id: int, invoice_number: int) -> str:
    """
    Genera un archivo PDF para un pedido dado y devuelve la ruta del archivo.
    """
    ...
    styles.add(ParagraphStyle(name='CustomTitle', parent=styles['h1'], fontSize=18, spaceAfter=30, alignment=1))
    ...
    if LOGO_PATH.exists():
        ...
        story.append(Paragraph("AndicBlue", styles['CustomTitle']))

    # Título y número de factura
    p_title = Paragraph(f"<b>FACTURA DE VENTA</b><br/>No. {invoice_number:03d}", styles['CustomTitle'])
    ...
    data_totals = [
        ...
        ["<b>Total a pagar:</b>", f"<b>{total:,}</b>".replace(',', '.')],
    ]
    ...
    p_footer = Paragraph("Gracias por confiar en AndicBlue 🍇 — Productos cultivados con orgullo nariñense", styles['Normal'])
                ...
                if st.button("Guardar cambios en pedido"):
                    try:
                        if not edited_items:
                            ...
                            # --- AJUSTE: Llamada a edit_order con el nuevo estado ---
...
# ---------------------------
    ...
    # Filtrar para mostrar solo pedidos entregados o con pagos
    ...
    if selected_order_option:
        ...
        if st.button("Generar Factura PDF", type="primary"):
            with st.spinner("Generando factura..."):
                try:
                    ...
                    pdf_path = generate_invoice_pdf(order_id, invoice_number)
                    st.success(f"¡Factura #{invoice_number:03d} generada con éxito!")
        ...
        if 'generated_pdf_path' in st.session_state and st.session_state['generated_pdf_path']:
            ...
            if os.path.exists(pdf_path):
                ...
                st.stop() # Detener si el archivo no existe

            # --- OPCIÓN 1: VISTA PREVIA MEJORADA ---
            st.markdown("#### Vista Previa en la página")
            try:
                with open(pdf_path, "rb") as pdf_file:
                    base64_pdf = base64.b64encode(pdf_file.read()).decode('utf-8')
                
                # Usamos st.components.v1.html que es más robusto
            ...
            # --- OPCIÓN 2: ABRIR EN NUEVA PESTAÑA (Muy fiable) ---
