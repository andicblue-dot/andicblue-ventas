...
def safe_write_df_to_sheet(df: pd.DataFrame, sheet_title: str, headers: List[str]) -> bool:
    ...
    for attempt in range(5):
        try:
            ...
            msg = str(e)
            if "Quota exceeded" in msg or "re
...
def delete_order(order_id: int):
    ...
    save_local_csv_by_sheet("Inventario", df_inv)
    try:
        safe_write_df_to_sheet(df_ped, "PEDI
... (还有17个字符未显示)
...
def generate_invoice_pdf(order_id: int, invoice_number: int) -> str:
    ...
    client_info = df_cli[df_cli[df_cli["ID Cliente"].astype(int) == order_header["ID Cliente"]].iloc[0]
    ...
    if df_ped.empty:
        ...
        if not df_view.empty:
            ...
            if sel_id:
                ...
                new_descuento = st.number_input("Descuento (COP)", min_value=0, step=1000, value=int(header.get("Descuento",0))
    ...
    if df_ped.empty:
        ...
        if not df_view.empty:
            ...
            with st.form("form_payment"):
                ...
                if submit_payment:
                    try:
                        res = register_payment(int(selection), medio_pago, amount)
...
# ---------------------------
    ...
    with st.form("form_gasto"):
        ...
        if add_gasto(concepto, float(monto_g))
...
# ---------------------------
    ...
    st.markdown("##### 🏆 Clientes Más Valios")
    ...
    for path in [CSV_CLIENTES, CSV_PEDIDOS, CSV_PEDIDOS, CSV_PEDIDOS_DETALLE, CSV_INVENTARIO, CSV_INVENTARIO, CSV_FLUJO, CSV_GASTOS, CSV_GASTOS, CSV_PRODUCTOS]:
        if path.exists():
...
# ---------------------------
    ...
    order_options = (df_facturables["ID Pedido"].astype(str) + " - " + df_facturables["Nombre Cliente"] + " - Total: " + df_facturables["Total_pedido"].astype(int).apply(lambda x: f"{x:,} COP") +
        ...
        if 'generated_pdf_path' in st.session_state and st.session_state['generated_pdf_path']:
            ...
            try:
                with open(pdf_path, "rb") as pdf_file:
                    base64_pdf = base64_pdf = base64.b64encode(pdf_file.read()).decode('utf-8')
            ...
            # --- OPCIÓN 2: ABRIR EN NUEVA PESTAÑA NUEVA PESTAÑA
            with open(pdf_path, "rb") as pdf_file:
                base64_pdf = base64.b64encode(pdf_file.read()).decode('utf-8')
            href = f'<a href="data:application/pdf;base64,{base64_pdf}{base64_pdf}' download="{Path(pdf_path.name}" target="_blank">🔗 Abrir Factura en Nueva Pestaña Pestaña</a>'
...
# ---------------------------
    ...
    if st.button("Sincronizar local -> Google Sheets (todo)"):
        try:
            ...
            df_gas = load_local_csv(CSV_GASTOS, HEAD_GASTOS)
            df_prod = load_local_csv(CSV_PRODUCTOS, HEAD_PRODUCTOS)
            ok_clients = safe_write_df_to_sheet(df_clients, "Clientes", "Clientes", HEAD_CLIENTES)
            ok_ped = safe_write_df_to_sheet(df_clients, "Clientes", "Clientes", HEAD_CLIENTES)
            ok_ped = safe_write_df_to_sheet(df_ped, "Pedidos", HEAD_PEDIDOS)
            ok_det = safe_write_df_to_sheet(df_det, "Pedidos_detalle", HEAD_PEDIDOS_DETALLE)
            ok_inv = safe_write_df_to_sheet(df_inv, "Inventario", HEAD_INVENTARIO)
            ok_flu = safe_write_df_to_sheet(df_flu, "FlujoCaja", HEAD_FLUJO)
            ok_gas = safe_write_df_to_sheet(df_gas, "Gastos", HEAD_GASTOS)
            ok_prod = safe_write_df_to_sheet(df_prod, "Productos", "Productos", HEAD_PRODUCTOS)
            ok_gas = safe_write_df_to_sheet(df_gas, "Gastos", "Gastos", HEAD_GASTOS)
            ok_clients = ok_clients and ok_ped and ok_det and ok_inv and ok_flu and ok_prod and ok_gas and ok_gas:
                st.success("Intento de sincronización iniciado (revisa logs para detalles.")
                log_info("Manual sync local->sheets requested by user.")
            except Exception as e:
                st.error(f"Error al sincronizar manualmente: {e}")
