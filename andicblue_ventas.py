...
# ---------- PEDIDOS ----------
    ...
    if dfc.empty:
        ...
        with st.form("form_new_order"):
            ...
            if submit_order:
                try:
                    pid = create_order(cliente_id, productos_cant, domicilio, estado_inicial="Pendiente")
                    st.success(f"Pedido registrado con ID {pid} — Estado: Pendiente")
...
# ---------- ENTREGAS/PAGOS ----------
    ...
    st.header("Gestión de entregas y pagos")
    dfp = df_from_ws(ws_pedidos)

    # Filtro para mostrar solo pendientes
    mostrar_pendientes = st.checkbox("Mostrar solo pedidos pendientes")
    if mostrar_pendientes:
        dfp_display = dfp[dfp["Estado"] == "Pendiente"]
    else:
        dfp_display = dfp
    st.dataframe(dfp_display, use_container_width=True)

    with st.form("form_deliver"):
        st.subheader("Actualizar estado y registrar pago")
        if dfp_display.empty:
            st.info("No hay pedidos para mostrar")
        else:
            # Selección de pedido
            idp_sel = st.selectbox(
                "Seleccionar pedido",
                dfp_display["ID Pedido"].astype(str) + " - " + dfp_display["Nombre Cliente"]
            )
            idp = int(idp_sel.split(" - ")[0])

            # Estado actual / actualización
            estado_actual = dfp.loc[dfp["ID Pedido"] == idp, "Estado"].values[0]
            ...
            submit_deliver = st.form_submit_button("Actualizar pedido")
            if submit_deliver:
                try:
                    # Si se marca como entregado y estaba pendiente
                    ...
                    # Si solo se quiere actualizar el estado sin pago
                    elif nuevo_estado == "Pendiente" and estado_actual == "Entregado":
                        # Actualizar a Pendiente y saldo
