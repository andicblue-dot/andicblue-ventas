...
def ensure_worksheet(ss, title, headers):
    try:
        ...
        ws.add_worksheet(title=title, rows="1000", cols="20")
        ws = ss.worksheet(title)
    try:
...
# ---------- ENTREGAS/PAGOS ----------
    ...
    st.header("Pagos y entregas")
    estado_filtro = st.selectbox("Filtrar pedidos", ["Todos", "Pendientes", "Entregados"])
    df_filtro = df_pedidos.copy()
    if estado_filtro!="Todos":
        df_filtro = df_filtro[df_filtro["Estado"]==estado_filtro]
    ...
    with st.expander("Registrar pago"):
        ...
        if st.button("Registrar pago"):
            try:
                ...
                st.success(f"Pedido {idp} actualizado. Productos={res['prod_paid']}, Domicilio={res['domicilio_paid']}, Saldo={res['saldo_total']}")
...
# ---------- FLUJO & GASTOS ----------
    ...
    st.dataframe(pd.DataFrame.from_dict(productos_totales, orient="index", columns=["Unidades vendidas"]))

# ---------- REPORTES ----------
elif menu=="Reportes":
    st.header("Reporte completo de pedidos")
    st.dataframe(df_pedidos, use_container_width=True)

st.caption("Nota: El valor del domicilio es fijo (3000 COP) y se contabiliza aparte del subtotal de productos.")
