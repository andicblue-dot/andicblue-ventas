...
CSV_PRODUCTOS = DATA_DIR / "productos.csv" # NUEVO
...
HEAD_PEDIDOS = [
    ...
    "Subtotal_productos", "Monto_domicilio", "Total_pedido", "Descuento", "Estado", # CAMBIO: Añadido Descuento
...
HEAD_PRODUCTOS = ["ID Producto", "Nombre", "Precio", "Costo"] # NUEVO
...
ensure_csv_with_headers(CSV_PRODUCTOS, HEAD_PRODUCTOS) # NUEVO
...
def load_local_csv_by_sheet(sheet_title: str) -> pd.DataFrame:
    ...
    elif sheet_title == "Productos": # NUEVO
...
def load_df(sheet_title: str) -> pd.DataFrame:
    mapping = {
        ...
        "Productos": (safe_read_sheet_to_df, HEAD_PRODUCTOS) # NUEVO
    }
...
def canonical_product_name(name: str) -> str:
    if not isinstance(name, str):
        return name
    s = name.strip()
    # NUEVO: Buscar en el DataFrame de productos
...
def create_client(nombre: str, tipo_doc: str, num_doc: str, telefono: str="", direccion: str="") -> int:
    ...
    # CAMBIO: Ordenar alfabéticamente antes de guardar
...
def edit_client(client_id: int, nombre: str, tipo_doc: str, num_doc: str, telefono: str="", direccion: str=""):
    ...
    # CAMBIO: Ordenar alfabéticamente después de editar
    dfc = dfc.sort_values(by='Nombre').reset_index(drop=True)
...
# NUEVO: Funciones CRUD para Productos
...
def create_order_with_details(cliente_id: int, items: Dict[str,int], domicilio_bool: bool=False, fecha_entrega: date=None, descuento: float=0) -> int:
    ...
    df_prod = load_df("Productos") # NUEVO

    subtotal = 0
    for p,q in items.items():
        prod = canonical_product_name(p)
        # NUEVO: Obtener precio desde el DataFrame de productos
    ...
    header_row = {
        ...
        "Subtotal_productos": subtotal, "Monto_domicilio": domicilio_monto, "Total_pedido": total, "Descuento": descuento, # CAMBIO
...
def edit_order(order_id: int, new_items: Dict[str,int], new_domic_bool: bool=None, new_week: int=None, new_estado: str=None, new_descuento: float=None):
    ...
    descuento = float(df_ped.at[idx_h, "Descuento"]) if new_descuento is None else new_descuento # CAMBIO
    total_new = (subtotal_new + domicilio) - descuento # CAMBIO
    ...
    df_ped.at[idx_h, "Descuento"] = descuento # CAMBIO
...
def register_payment(order_id: int, medio_pago: str, monto: float) -> Dict[str, float]:
    ...
    descuento_monto = float(df_ped.at[idx, "Descuento"]) # CAMBIO
    monto_anterior = float(df_ped.at[idx, "Monto_pagado"])
    monto = float(monto)
    nuevo_total = monto_anterior + monto

    total_a_pagar = (subtotal_products + domicilio_monto) - descuento_monto # CAMBIO
...
def generate_invoice_pdf(order_id: int, invoice_number: int) -> str:
    ...
    tbl_totals.setStyle(TableStyle([
        ('ALIGN', (0,0), (-1,-1), 'RIGHT'),
        ('FONTNAME', (0,3), (-1,3), 'Helvetica-Bold'), # CAMBIO: índice de la fila del total
        ('FONTSIZE', (0,3), (-1,3), 14),
...
def get_product_profitability_report(df_det: pd.DataFrame, df_prod: pd.DataFrame) -> pd.DataFrame:
    ...
    # Unir detalle de pedidos con productos para obtener el costo
...
if st.sidebar.button("🔁 Sincronizar local -> Sheets (manual)"):
    try:
        ...
        df_prod = load_local_csv(CSV_PRODUCTOS, HEAD_PRODUCTOS) # NUEVO
        ...
        ok_prod = safe_write_df_to_sheet(df_prod, "Productos", HEAD_PRODUCTOS) # NUEVO
...
if menu == "Dashboard":
    ...
    # NUEVO: Aplicar filtro de fechas
    if not df_ped.empty:
    ...
    if not df_flu.empty:
        ...
        # Aplicar filtro de fechas al flujo también
...
# ---------------------------
    ...
    with st.expander("✏️ Editar cliente existente"):
        if df_clients.empty:
            ...
            if selected_client_option != "-- Seleccionar --":
                ...
                client_data = df_clients[df_clients["ID Cliente"] == client_id_to_edit].iloc[0]
    ...
    if selected_client_option != "-- Seleccionar --" and selected_client_option:
        with st.expander(f"📜 Historial de Pedidos para: {client_data['Nombre']}"):
            df_ped = load_df("Pedidos")
            client_orders = df_ped[df_ped["ID Cliente"] == client_id_to_edit]
    ...
    with st.expander("✏️ Editar producto existente"):
        if df_productos.empty:
            ...
            if selected_product_option != "-- Seleccionar --":
                ...
                product_data = df_productos[df_productos["ID Producto"] == product_id_to_edit].iloc[0]
...
# ---------------------------
    ...
    df_det = load_df("Pedidos_detalle")
    df_inv = load_df("Inventario")
    df_productos = load_df("Productos") # NUEVO
    ...
    with st.expander("➕ Registrar nuevo pedido"):
        if df_clients.empty:
            ...
            # NUEVO: Obtener productos desde el DataFrame
    ...
    if df_ped.empty:
        ...
        if not df_view.empty:
            ...
            if sel_id:
                ...
                st.write(f"Descuento: {int(header.get('Descuento',0)):,} COP") # NUEVO
                ...
                # NUEVO: Campo de descuento en edición
                ...
                if st.button("Guardar cambios en pedido"):
                    try:
                        if not edited_items:
                            ...
                            edit_order(sel_id, edited_items, new_domic_bool=new_domic, new_week=new_week, new_estado=new_state, new_descuento=new_descuento)
...
# ---------------------------
    ...
    # NUEVO: Reporte de Rentabilidad por Producto
    ...
    for path in [CSV_CLIENTES, CSV_PEDIDOS, CSV_PEDIDOS_DETALLE, CSV_INVENTARIO, CSV_FLUJO, CSV_GASTOS, CSV_PRODUCTOS]: # NUEVO
        if path.exists():
...
# ---------------------------
    ...
    if st.button("Sincronizar local -> Google Sheets (todo)"):
        try:
            ...
            ok7 = safe_write_df_to_sheet(df_prod, "Productos", HEAD_PRODUCTOS) # NUEVO
