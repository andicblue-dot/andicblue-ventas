...
def load_sheet_to_df(sheet_title: str, cache_bust: int = 0) -> pd.DataFrame:
    ...
    for attempt in range(4):
        try:
            ...
            if df.empty:
                headers = []
                if sheet_title == "Clientes": headers = HEAD_CLIENTES
                elif sheet_title == "Pedidos": headers = HEAD_PEDIDOS
                elif sheet_title == "Pedidos_detalle": headers = HEAD_PEDIDOS_DETALLE
                elif sheet_title == "Inventario": headers = HEAD_INVENTARIO
                elif sheet_title == "FlujoCaja": headers = HEAD_FLUJO
                elif sheet_title == "Gastos": headers = HEAD_GASTOS
                return pd.DataFrame(columns=headers)
            
            # Asegurar que las columnas del DF coincidan con los HEAD_ definidos
            # Esto es importante si el sheet fue modificado manualmente o tenía errores
            expected_headers = []
            ...
            return pd.DataFrame(columns=expected_headers if 'expected_headers' in locals() else [])
        except Exception as e:
...
def coerce_numeric(df: pd.DataFrame, cols: List[str]):
    ...
    for c in cols:
        if c in df.columns:
            # Intentar convertir el tipo de la columna si es object y contiene algo numérico
...
def create_order(cliente_id: int, items: Dict[str,int], domicilio_bool: bool=False, fecha_entrega=None) -> int:
    ...
    if not df_clients.empty and "ID Cliente" in df_clients.columns:
        ...
        if cliente_id in cliente_id_series.values:
            ...
            raise ValueError(f"ID Cliente {cliente_id} no encontrado.")
